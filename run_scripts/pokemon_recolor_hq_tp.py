...
wgan = WGAN.PokeWGAN(IMAGE_SHAPE, './OUTPUT/pokeGAN_recolor_hq_tp/', log_tiling=(4, 4))
cache_dir = os.path.join('./cached_datasets/pokeGAN_recolor_hq_tp/', '{}/'.format(wgan.birthday))
if not os.path.exists(cache_dir):
    os.makedirs(cache_dir)
combined_ds = img_ds.cache(cache_dir).shuffle(5000, reshuffle_each_iteration=True).batch(16)

wgan.train(combined_ds, 2500, 0, 1, 10)
