...
# critic adapted from starGAN
    ...
    def _build_model(self, input_shape=[None, None, 3], n_classes=0, max_filters=2048, up_to_max=True):
        ...
        if n_classes > 0:
            cls = tf.keras.layers.Conv2D(n_classes, (h, w), strides=1, padding='valid')(x)
            cls_flattened = tf.keras.layers.Flatten()(cls)

            output = [src, cls_flattened]
        else:
            output = src

        return tf.keras.Model(inputs=inp, outputs=output, name=self.name)
